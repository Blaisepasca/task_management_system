...
class ValidateUserMiddleware
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        if (! $user->email_verified_at) {
            ...
            if ($request->ajax()) {
                return JsonResponse::fromJsonString(ResponseUtil::makeError('Your account is not verified.'),
        ...
        if (! $user->is_active) {
            ...
            if ($request->ajax()) {
                return JsonResponse::fromJsonString(ResponseUtil::makeError('Your account is deactivated. please contact your administrator.'),
                    Response::HTTP_UNAUTHORIZED);
            }
