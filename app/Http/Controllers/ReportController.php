...
class ReportController extends AppBaseController
{
    ...
    public function store(CreateReportRequest $request)
    {
        ...
        $reportArr = $this->reportRepository->store($input);
        $report = $reportArr[0];
        $input = $reportArr[1];
        ...
        $itemsArr = Arr::collapse(Arr::pluck($reports, 'items'));
        // dd($itemsArr);
        $input['report_id'] = $report->id;
        $input['project_id'] = $input['projectIds'];
        $input['name'] = $report->name.' '.$report->start_date.' - '.$report->end_date;
        $input['invoice_number'] = generateUniqueInvoiceNumber();
        $input['discount_type'] = '0';
        $input['amount'] = 0;
        $input['sub_total'] = 0;
        $input['discount'] = 0;
        $input['invoice_status'] = '1';
        $input['issue_date'] = $report->created_at;
        $input['total_hour'] = $totalHours;
        $input['itemsArr'] = $itemsArr;
    ...
    public function update(Report $report, UpdateReportRequest $request)
    {
        ...
        if ($invoiceExist) {
            $invoiceId = $report->reportInvoice()->value('invoice_id');
            Invoice::whereId($invoiceId)->delete();
