    ...
    public function getReport($report, $input = null)
    {
        ...
        foreach ($entries as $entry) {
            ...
            $item = [
                ...
                'item_name' => $entry->task->title.' ('.($entry->task->status ? "Completed" : "Pending").')',
                'item_project_id' => $project->id,
                'fix_rate' => 0,
                'duration' => $duration,
                'hours' => $this->getDurationTime($duration),
            ];
            ...
            $result[$deptId]['clients'][$clientId]['projects'][$project->id]['users'][$entry->user_id]['tasks'][$entry->task_id]['status'] = $entry->task->status;
        }
        // $result['itemsArr'] = $itemsArr;
