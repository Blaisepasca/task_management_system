...
let drake = dragula({
    ...
    $.ajax({
        url: taskUrl + '/' + id + '/update-task-status',
        type: 'PUT',
        data: {
...
$(document).on('blur', '.task-title-input', function (e) {
    ...
    $.ajax({
        url :  taskDetailsKanban + '/' + taskId +'/task-edit',
        type : 'post',
        data : {title : taskName},
        success: function (result){
...
$(document).on('click', '#btnCommentSave', function () {
    ...
    if (editMode === 0)
        ...
        commentUrl = baseUrl + 'tasks/' + taskId + '/comments/' + commentId + '/update';
    $.ajax({
        url: commentUrl,
        type: 'post',
...
function handleFileSelect(e){
    ...
    for(let i = 0; i < files.length; i++) {
        ...
        if ($.inArray(ext, [
            ...
            $.ajax({
                url: taskDetailsKanban + '/' + taskId + '/add-attachment',
                type: 'post',
                data: formData,
                cache: false,
                contentType: false,
                ...
                success: function (result) {
                    ...
                    if (result.success) {
                        ...
                        let ext = attachments.attachment.file_name.split('.').
                            pop();
                        let image = [];
                        let attachments_data = [];
                        ...
                        attachments_data = [
                            {
                                ...
                                'downloadTask': downloadTasks,
                                'updated_at': moment(
                                    attachments.attachment.updated_at).
                                    fromNow(),
                            }];
                        $('#attachments').
                            append(prepareTemplateRender('#attachment',
                                attachments_data));
                    }
                    // }
