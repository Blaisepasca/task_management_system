$(document).ready(function () {
    ...
    $(document).on('change', '#clientSelectBox', function () {
        ...
        if (clientId !== '') {
            $.ajax({
                url: getClientProjects + clientId + '/projects',
                type: 'GET',
    ...
    $(document).on('change', '.projects-select-box', function () {
        ...
        $.ajax({
            url: getProjectTasks + projectId + '/tasks',
            type: 'get',
            success: function (result) {
                $.each(result.data, function (index, value) {
                    let data = [
    ...
    $(document).on('change', '.task-select-box', function () {
        ...
        if (taskId !== '') {
            $.ajax({
                url: getTaskDetails + taskId + '/details',
                type: 'get',
                success: function (result) {
    ...
    $(document).on('keyup', '.task-amount', function () {
        ...
        if ($(this).val() != '') {
            $(this).val(getFormattedPrice(taskAmount));
        } else {
            $(this).val(0);
        }
        calculateSubTotal();
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.success) {
                    ...
                    window.location.href = clientInvoicesUrl + '/' + result.data.id;
                }
            },
            error: function (result) {
                screenUnLock();
    ...
    $(document).on('click', '.delete-btn', function (event) {
        ...
        deleteItem(invoiceUrl + '/' + invoiceId, 'tableName', 'Invoice',
            'location.reload()');
    });
