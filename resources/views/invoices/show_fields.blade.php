<div class="card-body">
    ...
    <div class="row">
        <div class="col-lg-3 col-sm-6 form-group">
            ...
            <p class="d-table-cell">{{ html_entity_decode($invoice->name) }}</p>
        </div>
        <div class="col-sm-3 form-group">
            ...
            <p class="d-table-cell">
                @forelse($invoice->invoiceClients->pluck('name') as $client)
                    {{$loop->first ? '':', '}}
                    <span>{{html_entity_decode($client)}}</span>
                @empty
        ...
        <div class="col-sm-3 form-group">
            ...
            <span
                class=" d-table-cell">{{ isset($invoice->issue_date) ? Carbon\Carbon::parse($invoice->issue_date)->format('jS M, Y') : 'N/A' }}</span>
        </div>
        <div class="col-sm-3 form-group">
            <span class="font-weight-bold">{{ __('messages.invoice.due_date').':' }} </span><br>
            <span
                class=" d-table-cell">{{ isset($invoice->due_date) ? Carbon\Carbon::parse($invoice->due_date)->format('jS M, Y') : 'N/A' }}</span>
        </div>
        <div class="col-sm-3 form-group">
            <span class="font-weight-bold">{{ __('messages.tax.tax').':' }} </span>
            <p class=" d-table-cell">{{ isset($invoice->tax_id) ? html_entity_decode($invoice->tax->name).'('.$invoice->tax->tax.'%)' : 'N/A' }}</p>
        </div>
        <div class="col-sm-3 form-group">
        ...
        <div class="col-sm-12 form-group">
            ...
            <p>{!! isset($invoice->notes) ? html_entity_decode(nl2br(e($invoice->notes))) : 'N/A' !!}</p>
        </div>
    </div>
</div>
<hr>
        ...
        <div class="table-responsive">
            <table class="table table-striped table-bordered">
                ...
                @foreach($invoice->invoiceItems as $item)
                    <tr>
                        <td><span>{{ isset($item->item_name) ? html_entity_decode($item->item_name) : '' }}</span></td>
