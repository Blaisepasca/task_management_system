                    ...
                    @foreach($invoice->invoiceClients as $client)
                        @foreach($client->projects as $project)
                            <option class="new-option"
                                    value="{{ $project->id }}" {{ $invoice->invoiceProjects->contains($project->id) ? 'selected' : '' }}>{{ html_entity_decode($project->name) }}</option>
                        @endforeach
                    @endforeach
                </select>
            </div>
...
<div>
        ...
        <div class="table-responsive">
            <table class="table table-striped table-bordered">
                ...
                @foreach($invoice->invoiceItems as $invoiceItem)
                    <tr>
                        <td>
                            <a href="{{ (isset($invoiceItem->task))?url('tasks',$invoiceItem->task->project->prefix.-$invoiceItem->task->task_number):'#' }}"
                               target="_blank">
                                <input type="text" name="item[]"
                                       value="{{ isset($invoiceItem->item_name) ? html_entity_decode($invoiceItem->item_name) : '' }}"
                                       class="form-control item-name input-group__icon" required="" readonly></a>
    ...
    <div class="form-group col-sm-12">
        <div class="btn-group dropup open">
            ...
            @else
                {{ Form::button(__('messages.invoice.save_and_send'), ['class' => 'btn btn-primary', 'id' => 'editSaveAndSend', 'data-status' => '1', 'data-loading-text' => "<span class='spinner-border spinner-border-sm'></span> Processing..."]) }}
